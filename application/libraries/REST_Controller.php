...
abstract class REST_Controller extends \CI_Controller {
	...
	public function check_valid_user()
	{
		...
		if(isset($request_header['Access-Token']) && !empty($request_header['Access-Token'])){
			$token=$request_header['Access-Token']; 
			$response=$this->GetTokenData($token);
			if(!empty($response['user_id']))
			{
				$result=$this->Md_database->getData(USERS,'*',array('user_id'=>$response['user_id'],'is_active'=>'active','is_delete'=>'active'));
				if(!empty($result[0]))
				{
	...
	# Jwt Token Start
    public function LoginToken($user_id,$email){
